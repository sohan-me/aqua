...
export default function MortalityPage() {
  ...
  const handleDelete = async (
    ...
    if (
      window.confirm(
        `Are you sure you want to delete the mortality record for ${pondName} (${count} fish) on ${formatDate(
          date
  ...
  const totalMortalities = mortalities.reduce(
    (sum, mortality) => sum + mortality.count,
    0
  );
  const totalStocked = stockings.reduce(
    (sum, stocking) => sum + stocking.pcs,
    0
  );
  const mortalityRate =
    totalStocked > 0 ? (totalMortalities / totalStocked) * 100 : 0;
  const avgMortalityPerDay =
    mortalities.length > 0 ? totalMortalities / mortalities.length : 0;
      ...
      <div className="md:flex space-y-3 md:space-y-0 items-center justify-between">
        <div>
          ...
          <p className="text-sm md:text-base text-gray-600">
            Monitor fish mortality rates and causes
          </p>
        </div>
        <Link
          style={{ color: "white" }}
      ...
      {mortalities.length === 0 ? (
        <div className="text-center py-12">
          ...
          <p className="mt-1 text-sm text-gray-500">
            Get started by recording your first mortality event.
          </p>
          <div className="mt-6">
            <Link
              href="/mortality/new"
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {mortalities.map((mortality) => (
                  <tr key={mortality.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                      {mortality.avg_weight_kg
                        ? `${formatWeight(
                            parseFloat(mortality.avg_weight_kg)
                          )} kg`
                        : "N/A"}
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
